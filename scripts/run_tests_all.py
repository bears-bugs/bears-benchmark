...
parser = argparse.ArgumentParser(description='Script to run tests related to bugs checked out from Bears')
...
for BUG_FOLDER in BUG_FOLDERS:
    if BUG_FOLDER.startswith('Bears'):
        os.system("python %s/scripts/run_tests_bug.py --bugId %s --workspace %s" % (BEARS_PATH, BUG_FOLDER, WORKSPACE))

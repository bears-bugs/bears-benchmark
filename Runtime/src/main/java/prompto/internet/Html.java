...
public class Html {
		...
		// copied from org.apache.commons.lang.Entities.java
	...
	private static final Map<Character, String> charToEntityMap = getCharToEntityMap();
	private static final Map<String, Character> entityToCharMap = getEntityToCharMap();

	private static Map<Character, String> getCharToEntityMap() {
		Map<Character, String> map = new HashMap<>();
		for (String[] pair : ENTITIES) {
			map.put((char) Integer.parseInt(pair[1]), "&" + pair[0] + ";");
		}
		return map;
	}

	private static Map<String, Character> getEntityToCharMap() {
		Map<String, Character> map = new HashMap<>();
		for (String[] pair : ENTITIES) {
			map.put("&" + pair[0] + ";", (char) Integer.parseInt(pair[1]));
		}
		return map;
	}
	...
	public static String encode(String value) {
		...
		for (char c : value.toCharArray()) {
			String s = charToEntityMap.get(c);
			if (s != null)
				sb.append(s);
			else
				sb.append(c);
	...
	public static String decode(String value) {
		...
		int nextIdx = 0;
		for(;;) {
			// run to next entity start
			int idx = nextIdx;
			for(; idx<chars.length && chars[idx]!='&'; idx++)
				;
			// append regular chars
			if(idx>nextIdx)
				sb.append(chars, nextIdx, idx-nextIdx);
			// done?
			if(idx>=chars.length)
				break;
			// entity found, run to next entity end
			nextIdx = idx;
			for(; idx<chars.length && chars[idx]!=';'; idx++)
				;
			// found one?
			...
			// decode entity
			if(chars[nextIdx+1]=='#') {
				String charCode = new String(chars, nextIdx+2, idx-nextIdx-2);
				sb.append((char)Integer.parseInt(charCode));
			} else {
				String entity = new String(chars, nextIdx, 1+idx-nextIdx);
				sb.append(entityToCharMap.get(entity));
			}
			// and again
			nextIdx = idx + 1;
		}

...
/**
 * <pre>
 * -webkit-flex-basis: <i>length</i>|auto|initial|inherit;
 *
 * The -webkit-flex-basis property specifies the initial length of a flexible item.
 *
 * Note: If the element is not a flexible item, the -webkit-flex-basis property has no effect.
 * Default value:  auto
 * Inherited:      no
 * Animatable:     yes
 * Version:        CSS3
 * JavaScript syntax:      object.style.webkitFlexBasis="200px"
 * </pre>
 *
 * @author WFF
 * @since 1.0.0
 */
public class WebkitFlexBasis extends AbstractCssProperty<WebkitFlexBasis> {
     ...
     */
    public WebkitFlexBasis(final String cssValue) {
        setCssValue(cssValue);
    }

    /**
     * @param webkitFlexBasis
     *            the {@code WebkitFlexBasis} object from which the cssValue to
     *            set.And, {@code null} will throw {@code NullValueException}
     */
    public WebkitFlexBasis(final WebkitFlexBasis webkitFlexBasis) {
        if (webkitFlexBasis == null) {
            throw new NullValueException("webkitFlexBasis can not be null");
        }
        setCssValue(webkitFlexBasis.getCssValue());
    }
     ...
     * @author WFF
     */
    public WebkitFlexBasis(final float percent) {
     ...
     * @param cssLengthUnit
     */
    public WebkitFlexBasis(final float value,
     ...
     * @author WFF
     */
    public WebkitFlexBasis setValue(final float value,
            final CssLengthUnit cssLengthUnit) {
        this.value = value;
        this.cssLengthUnit = cssLengthUnit;
        cssValue = String.valueOf(value) + cssLengthUnit;
    ...
    public String getCssName() {
        return CssNameConstants.WEBKIT_FLEX_BASIS;
    }
    ...
    /**
     * gets the width in float value. {@code WebkitFlexBasis#getUnit()} should
     ...
     */
    @Override
    public WebkitFlexBasis setCssValue(final String cssValue) {
        try {
            if (cssValue == null) {

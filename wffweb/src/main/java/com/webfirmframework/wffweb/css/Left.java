 ...
 *
 * For relatively positioned elements, the left property sets the left edge of an element to a unit above/below its normal position.
 *
 ...
 * @since 1.0.0
 */
public class Left extends AbstractCssProperty<Left> {
    ...
    public Left(final Left left) {
        if (left == null) {
            throw new NullValueException("left can not be null");
        }
        setCssValue(left.getCssValue());
    }
    ...
    /**
     * gets the width in float value. {@code Left#getUnit()} should be used to
     ...
     */
    @Override
    public Left setCssValue(final String cssValue) {

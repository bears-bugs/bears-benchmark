 ...
 * @since 1.0.0
 */
public class HeightCss extends AbstractCssProperty<HeightCss> {
    ...
    public HeightCss(final String cssValue) {
        setCssValue(cssValue);
    }

    /**
     * @param heightCss
     *            the {@code HeightCss} object from which the cssValue to
     *            set.And, {@code null} will throw {@code NullValueException}
     */
    public HeightCss(final HeightCss heightCss) {
        if (heightCss == null) {
            throw new NullValueException("heightCss can not be null");
        }
        setCssValue(heightCss.getCssValue());
    }

    public HeightCss(final float percent) {
        cssLengthUnit = CssLengthUnit.PER;
        value = percent;
        cssValue = String.valueOf(percent) + cssLengthUnit;
    }

    public HeightCss(final float value, final CssLengthUnit cssLengthUnit) {
        this.value = value;
    ...
    public String getCssName() {
        return CssNameConstants.HEIGHT;
    }
     ...
     */
    @Override
    public HeightCss setCssValue(final String cssValue) {
        try {
            if (cssValue == null) {
